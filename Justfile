set dotenv-filename := "bazzitttte.env"
...
# Build + push the image to GHCR manually (fast iteration without CI); needs `podman login ghcr.io`, then apply on the NUC with `ujust bazzitttte-update`
publish $target_image=image_name $tag=default_tag: (build target_image tag)
    ...
    echo "On the NUC, apply it with: ujust bazzitttte-update"
...
lint:
    ...
    # Run shellcheck on all Bash scripts (plus extensionless bash files)
    find . -iname "*.sh" -type f -exec shellcheck "{}" ';'
    shellcheck system_files/usr/libexec/bazzitttte-autologin
